...
global  q, epsilon, m, n, a, o,with_obstacle, bel, b_tmp, cos, sin,bel_dict, Probability
...
def reward(state):
    if (with_obstacle == 0):
        ...
        if ((state[0] < 0 or state[0] >= n) or (state[1] < 0 or state[1] >= m)): # check it out whether we are out of grid or not
            ...
            #if (((state[0] <= 10 and state[0]>= 8) and (state[1] <= 3 and state[1] >=0 )) or ((state[0] <= 10 and state[0]>= 8) and (state[1] <= (m-1) and state[1] >=((m-1)-2) ))):
...
#initializing
with_obstacle = 1 # our grid world has obstacles
...
#Q=[[[[0 for h in range(a)] for k in range(o)] for i in range(m)] for j in range(n)]
...
while (episode < 2000):
    ...
    while (goal == 0):
        ...
        #--------------------------Q(b,a) = sum (b(s) * q(s,a))----------------------------------
        Q_s = Q_s_a(action)
        my_bel =  bel_dict[(current_state[0],current_state[1],current_state[2])]
        ...
        for w in range(4):
            value = Q_s_a(w)
            lst.append(value)
        tmp = q[current_state[0]][current_state[1]][current_state[2]][action]
        q[current_state[0]][current_state[1]][current_state[2]][action] = tmp + ALPHA * my_bel * (rwd + GAMMA * max(lst) - Q_s)
